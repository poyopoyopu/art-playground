- 2026-09-25 クロード: PRISM NAVEのタッチの法則を作り替え(おっちゃん「タッチ効果が面白くない」)。「触れた場所が消失点になる」をやめ、「押さえた線から、時間が流れ出す」に。押さえた一列がスリットになり、そこを通った瞬間の世界が外側へ押し出され続けて、時間が空間に引き伸ばされた帯になる(スリットスキャン)。中央から横に離れた所は縦のスリット、地平線から上下に離れた所は横のスリットで、左右・上下は鏡写しで同時に起きる。指を動かすとスリットも動き、離すと帯は止まった時間として残る。✦は虹の種類と建築を変える(止まった帯は残る)、↻で止まった時間を全部消す(prototypes/prism-nave.html)
- 2026-09-25 クロード: PRISM NAVEの白飛びを抑制。地平線の色を白っぽい黄から橙〜黄へ、遠近の重ね方を加算(screen)から明るい方を取る合成(lighten)へ変え、染めた後に足す白・にじみ・消失点で交わる芯と光の筋・誕生の閃光を弱めた(法則と動きは変更なし)(prototypes/prism-nave.html)
- 2026-09-25 クロード: おっちゃん提供のInstagramリール(左右対称のガラス建築が虹色の帯の中を奥から流れてくる映像)を参考にPRISM NAVEを新規制作。法則は「触れた場所が消失点になる」。ガラスの箱・塔・光の筋はすべて一つの消失点へ向かう遠近で建ち、消失点で左右対称・地平線で上下に映り込み、虹の帯はその地平線を基準に染まる。触れるとそこが新しい消失点になって新しい遠近の建築が湧き、古い遠近も消えずに自分の消失点へ流れ続けるので、触るほどありえない遠近が重なる(最大5つ、6つ目で最古が溶ける)。スリットスキャン風の画素の引き伸ばし付き。✦で虹の種類を変えて建て直し、↻で中央の一つだけに戻す(prototypes/prism-nave.html)
...
- 2026-09-24 クロード: おっちゃん提供のX投稿GIF(光る核のまわりを点線の軌道が何重にも傾いて回る映像)を参考にONE PLANEを新規制作。軌道をつかんで面を傾けられ、別の軌道とほぼ同じ面にそろうと噛み合ってひとつの面になり、二度と離れない(フレアが出て核が重く明るくなる)。触るほど立体の雲が平たくなり、最後は一枚の円盤に落ち着く。何もない所のドラッグは視点の回転(prototypes/one-plane.html)
